...
персоналий через админку «🎭 Персоналии» (тексты в Neon, не в git). ⚠️ P14 добавил env `HF_TOKEN` (write-токен HF,
добавить руками в секреты Space — см. DEPLOY_NOTES); без него кнопка рестарта
дизейблится, остальное работает. Q-NET.b добавил опц. env-тюнеры пула/таймаутов
(`BOT_KEEPALIVE_TIMEOUT`/`BOT_TOTAL_TIMEOUT`/`BOT_SOCK_CONNECT_TIMEOUT`/
`BOT_FORCE_CLOSE`) — все с дефолтами, руками выставлять НЕ обязательно.
Прочее — в `admin_config`.
...
> **Статус (2026-06-10):** Q-NET.b реализован и задеплоен (см. ниже).
> Остаются a/c/d — по ним план готов, но кодить только после согласования
> с пользователем (особенно d — переключатель транспорта, большой объём).
...
- [x] **Q-NET.b (10в).** (2026-06-10) Таймауты и пул соединений: keepalive_timeout=15
  + гранулярный `ClientTimeout(total=25, sock_connect=8)` на direct/proxy-коннекторе
  (env-тюнеры `BOT_KEEPALIVE_TIMEOUT`/`BOT_TOTAL_TIMEOUT`/`BOT_SOCK_CONNECT_TIMEOUT`,
  все с дефолтами); `BOT_FORCE_CLOSE=true` — аварийный режим «без пула» (keepalive
  не передаётся, иначе aiohttp ValueError). Направление proxy↔direct не трогаем (Q8).
  Тесты `tests/test_net_pool.py` (8). HF `ef08ca5`, GitHub `41addb8`. Сьют 370.
...
- [~] **Q-NET.e.** Тесты+sync для .b сделаны (см. выше). Остаётся для a/c/d
  + DEPLOY_NOTES, когда они будут реализованы.
...
- [x] **P4.1.f.** (2026-06-09) Тесты `test_welcome_prefs.py` (12 шт., сьют
  **344 passed**), sync `backend/` → `meetup-planner-backend/`, push:
  HF `1f9ae95`, GitHub/Pages `97e1b2f` (пушил ассистент, PAT).

### P6. Генератор фраз с типажами
Источник: GHG7.txt стр. 151–179.
- [x] **P6.1.a.** (2026-06-09) Neon-таблица `participant_personas` (user_id PK,
  persona_text, updated_at) — миграция `0015_participant_personas`, модель
  `ParticipantPersona`. Нагрузка нулевая: SELECT только при генерации/в
  админке. CRUD: `GET /admin/personas` (+счётчики шаблонов/битых),
  `PUT /admin/personas/{uid}` (пустой текст = удаление),
  `POST /admin/personas/{uid}/preview` (превью без сохранения). UI —
  `PersonasScreen.tsx` (Card «🎭 Персоналии» в AdminScreen).
- [ ] **P6.1.b.** Сидинг 6 персоналий (GHG7.txt стр. 154–159) — руками
  пользователя через админку («🎭 Персоналии», формат [шаблоны]/[слоты]).
- [x] **P6.1.c.** (2026-06-09) Sync — общий с P6.3.b ниже.
- [x] **P6.2.a.** (2026-06-09) `services/personas.py`: `parse_persona`
  (секции `[шаблоны]`/`[слот]`, `{плейсхолдеры}`, комменты `#`),
  `render_phrase` (битые шаблоны отбрасываются), `weighted_pick_user`
  (вес = 1 + сообщений за окно; молчуны не выпадают из ротации),
  `compose_persona_phrase` (формат вывода как v1 «👤 Имя вещает»). Без LLM.
- [x] **P6.2.b.** (2026-06-09) v2 встроен в `run_random_phrases_job` —
  расписание/шанс/ручной триггер/кнопка «запустить сейчас» общие; при
  отсутствии пригодных персоналий — авто-фолбэк на legacy (пост не срывается).
- [x] **P6.2.c.** (2026-06-09) Sync — общий с P6.3.b ниже.
- [x] **P6.3.a.** (2026-06-09) `phrase_generator.version` ∈ {legacy, personas}
  (default legacy) в admin_config; поле `generator_version` в
  GET/PUT `/admin/random-phrases/generator` (default legacy — старые клиенты
  совместимы); сегмент «🎭 Версия генератора» в GeneratorBody.
- [x] **P6.3.b.** (2026-06-09) Тесты `test_personas.py` (18 шт., сьют
  **362 passed**), sync `backend/` → `meetup-planner-backend/`, push:
  HF `f4b0fbf` (миграция 0015 применится при старте), GitHub/Pages `9e685fd`
  (пушил ассистент, PAT).
...
- [x] **P2.1.c.** (2026-06-10) Клик по 🪱-бейджу под аватаркой → попап-история
  звания «Червь-пидор» (общая, не per-user — звание переходящее). Backend:
  публичный `GET /api/worm/history` (worm_assignments DESC, ended_at=null =
  текущий). Frontend: `fetchWormHistory`, `showWormHistory` в UI-сторе,
  `WormHistoryPopover.tsx` (имена из кэша `["users"]`, активная строка «сейчас»).
  HF `ef08ca5`, GitHub `41addb8`. (Чухан/ДР-истории через клик-шапку — не делали:
  чухан уже виден в Профиле/ячейках, отдельный поп-ап избыточен.)
