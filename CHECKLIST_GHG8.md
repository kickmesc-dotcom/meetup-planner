...
- [ ] **Q-INV-1 (Q5). Причины чухана: 6 старых, правки не видны.**
  Проверить прямым коннектом к Neon (`DATABASE_URL`, asyncpg): что лежит в
  `admin_config` под `chukhan_reasons.list` и `chukhan_reasons.use_counts`.
  Гипотезы: (а) ключ содержит старый список из 6 — правка `chukhan.py` by
  design не применяется; (б) пользователь правил не тот ключ/невалидный JSON
  → `get_chukhan_reasons` молча падает на дефолт. Сверить, почему у лоха
  правки появились (был ли ключ `loser_reasons.list`). **Вывод + follow-up в
  Q5.**
...
- [ ] **Q5.a.** По выводу Q-INV-1: если в БД старый список — решить UX. Варианты:
  (1) кнопка в админ-UI «сбросить к дефолтам из кода»; (2) сидинг новых причин
  пользователем через UI (как задумано — дефолты в коде только до первой
  правки). Зафиксировать, что правка `chukhan.py` в проде НЕ применяется при
  наличии ключа — это ожидаемо, не баг кода.
- [ ] **Q5.b.** Если корень — невалидный JSON/тихий фолбэк: добавить лог/диаг
  в `get_chukhan_reasons` (как `random-phrases/pool/raw` в P0.3), чтобы видеть
  фактический размер списка и причину фолбэка.
- [ ] **Q5.c.** Sync.
...
### Q-прочее (зафиксировано, действий по коду не требует)
- **Q1.** Скип чухана в понедельник (коннект) — ЗАКРЫТО блоком **P11**
