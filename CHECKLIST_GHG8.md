...
- [x] **P3.1.e.** (2026-06-06) `tests/test_birthday_immunity.py` — 12 новых:
  ...
  HistoryScreen).
- [x] **P3.1.f.** (2026-06-07) Sync: backend → `meetup-planner-backend/`
  (274 passed в обеих копиях). Запушено: HF `2c13d2c`, GitHub/Pages-source
  `3f01ab1` (фронт пересоберётся автоматически). **HF env-напоминание:**
  новых env НЕ требуется (immunity_mode — в `admin_config`, таймаут —
  существующий `LOSER_SEND_TIMEOUT`).
...
- [x] **P7.1.a.** (2026-06-07) Решение с пользователем (AskUserQuestion): НЕ
  таблица+миграция, а **admin_config JSON** (паттерн P5/Q7.b — фраз немного).
  Новый модуль `services/dead_chat.py`: пороги THRESHOLDS (24h/72h/week/month/
  half_year/year), ключи `dead_chat.enabled` (default true) / `.phrases`
  (JSON {threshold: [фразы]}, дефолты порога из кода при отсутствии/кривом
  значении — паттерн loser_reasons) / `.last_activity_at` (ISO) / `.last_post`
  (JSON {threshold, activity_at} — якорь анти-спама). История ChatMessage НЕ
  используется (retention 7 дней < порогов до года) — источник правды о
  тишине только персистнутая метка. **Активность = текст + медиа** (решение
  пользователя): `touch_chat_activity` зовётся из `chat_capture.py` (текст) и
  `media_reactions._on_media_impl` (медиа, после whitelist-фильтров);
  in-memory троттлинг 15 мин — не чаще одного UPSERT в Neon, best-effort.
- [x] **P7.1.b.** (2026-06-07) Seed = `DEFAULT_PHRASES` в коде (сидить в БД
  нечего — дефолты подхватываются при отсутствии ключа): 24h «Я что-то грубое
  сказал?»/«дохлая корова»/«промолчи если не любишь маму» (цитаты GHG7.txt
  дословно) → … → year «радиоактивные осадки» (философское, дословно). 5/4/4/
  3/3/3 фраз по порогам, эскалация тона.
- [x] **P7.1.c.** (2026-06-07) `scheduler.py`: job `dead_chat_hourly`
  (IntervalTrigger 1ч, jitter 120, misfire_grace 1ч, вне reload_dynamic_jobs —
  enabled проверяется внутри, как bot_pause_auto_restore).
  `run_dead_chat_job`: скип при паузе бота (`is_paused` — /zaebal гасит и
  пинки), первый запуск инициализирует метку «сейчас» (нет фантомной тишины
  года после деплоя), битая метка — самовосстановление. Выбор порога —
  `pick_threshold`: САМЫЙ ГЛУБОКИЙ достигнутый (офлайн-скачок 24h→72h даёт
  один пост, не очередь); анти-спам — один пост на порог за окно тишины
  (last_post.activity_at == текущая метка), эскалация внутри окна разрешена,
  новая активность открывает окно заново. Send через wait_for (env
  `LOSER_SEND_TIMEOUT`, 25с); при фейле last_post не пишется — следующий
  часовой тик ретраит сам. Фейл send → лог, не исключение. Master-toggle
  проведён в `get/set_scheduled_settings` + `ScheduledDeadChatIO`
  (`routes_admin.py`, default_factory — старые клиенты не сбрасывают) +
  тогглер «🪦 Пинок мёртвого чата» в `ScheduledPublicationsScreen` (`?? true`
  для старых серверов).
- [x] **P7.1.d.** (2026-06-07) Тесты: `tests/test_dead_chat.py` — 38 новых
  (parse_phrases: дефолты/кривой JSON/частичная кастомизация/strip/unknown;
  pick_threshold: достижение порогов, офлайн-скачок → один пост по глубокому,
  анти-спам окна, эскалация, сброс новой активностью; env-таймаут; инварианты
  дефолтных пулов). Сьют **312 passed** (было 274), tsc чист (baseline
  HistoryScreen). Sync: backend → `meetup-planner-backend/` (312 passed).
  Запушено: HF `d7c5c20`, GitHub/Pages-source `4db9223`.
  **HF env-напоминание:** новых env НЕ требуется (всё в `admin_config`,
  таймаут — существующий `LOSER_SEND_TIMEOUT`).
  ⚠️ Push в GitHub через credential.helper=manager ВИСНЕТ из этой сессии
  (молча, без вывода, >5 мин) — пушить с PAT в URL:
