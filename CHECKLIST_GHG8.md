...
- [ ] **P14-INV.** Проверить вариант A на живом Space (curl с hf-token из
  `leltokens2.txt`): рестартует ли, какой даунтайм, что с вебхуком после
  подъёма (должен переустановиться retry-логикой старта). Зафиксировать выбор
  A или B.
- [ ] **P14.1.a.** Backend: `POST /admin/space/restart` — подтверждаемый
  рестарт по кнопке. Ответ уходит клиенту ДО фактического рестарта (иначе
  фронт получит обрыв). Лог `space_restart.requested` с источником (manual).
- [ ] **P14.1.b.** Frontend: кнопка «🔄 Рестарт Space» в админке (секция
  Прокси/Сеть или отдельная «Сервис») с `showConfirm` + понятный тост
  «Space перезапускается, ~1–2 мин даунтайм».
- [ ] **P14.2.a.** Расписание: `admin_config[space_restart.schedule]` —
  `{mode: off|once|interval, at: ISO (для once), every_hours: N (для
  interval)}`. Scheduler-job проверяет расписание (паттерн
  bot_pause_auto_restore: IntervalTrigger, enabled внутри). После
  однократного (`once`) — режим сбрасывается в off ДО рестарта (иначе
  рестарт-луп). Для interval — якорь `last_restart_at` в admin_config,
  чтобы рестарт не сдвигался от каждого подъёма Space.
- [ ] **P14.2.b.** Frontend: UI расписания в той же секции (once: дата-время;
  interval: N часов/дней), отображение «следующий рестарт: …».
- [ ] **P14.3.** Гарды: не рестартовать во время активной TG-операции не
  требуется (best-effort), но НЕ рестартовать чаще раза в 30 мин (анти-луп,
  кламп в сеттере) + лог `space_restart.scheduled_fired`.
- [ ] **P14.4.** Тесты (парсер расписания/клампы/сброс once) + sync +
  DEPLOY_NOTES (env `HF_TOKEN`, если выбран вариант A).
